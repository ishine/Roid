...
class Encoder(nn.Module):
    def __init__(self, channels, n_heads, n_layers, kernel_size=1, p_dropout=0.,
        ...
        for i in range(self.n_layers):
            self.attn_layers.append(
                MultiHeadAttention(channels, channels, n_heads, window_size=window_size,
                                   p_dropout=p_dropout, block_length=block_length))
            self.norm_layers_1.append(LayerNorm(channels))
            self.ffn_layers.append(
                FFN(channels, channels, kernel_size, p_dropout=p_dropout))
...
class FFN(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, p_dropout=0., activation=None):
        ...
        self.conv_1 = nn.Conv1d(in_channels, in_channels * 4, kernel_size, padding=kernel_size // 2)
        self.conv_2 = nn.Conv1d(in_channels * 4, out_channels, kernel_size, padding=kernel_size // 2)
        self.drop = nn.Dropout(p_dropout)
